...
# Install them for the build user and put the user script dir on PATH.
...
# same archive is also linked by the _zstd Python C extension via
# `-l:libzstd.a` in Modules/Setup, so the final binary only contains one
# copy of zstd's code.
(
  cd dep-build
